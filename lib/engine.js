...
module.exports = function (s, conf) {
    ...
    s.exchange.getBalance({currency: s.currency, asset: s.asset}, function (err, balance) {
      ...
      s.exchange.getQuote({product_id: s.product_id}, function (err, quote) {
        ...
        if (!s.start_capital) {
          ...
          s.start_asset = n(s.balance.currency).add(n(s.balance.asset).multiply(quote.ask)).value()
      ...
      getQuote(function (err, quote) {
        ...
        if (signal === 'buy') {
          ...
          if(so.sell_min_amt && so.sell_min_amt > s.asset_capital) {
            let err = new Error('\nasset protection')
            err.desc = 'refusing to sell at asset capital ' + fa(s.asset_capital)
            return cb(err)
          }
  ...
  function writeReport (is_progress, blink_off) {
    ...
    process.stdout.write('  ' + fc(s.period.close, true, true, true) + ' ' + s.product_id.cyan)
    if (s.lookback[0]) {
...
*/
      let profit = (s.balance.asset - s.start_asset) / s.start_asset
