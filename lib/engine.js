...
let clock
let nice_errors = new RegExp(/(slippage protection|loss protection|asset protection)/)
...
module.exports = function (s, conf) {
  ...
  function executeSignal (signal, _cb, size, is_reorder, is_taker) {
    ...
    syncBalance(function (err, { quote }) {
      ...
      if (signal === 'buy') {
        ...
        if(so.sell_min_amt && so.sell_min_amt > s.asset_capital) {
          let err = new Error('\nasset protection')
          err.desc = 'refusing to sell at asset capital ' + formatCurrency(s.asset_capital, s.currency)
          return cb(err)
        }
  ...
  function writeReport (is_progress, blink_off) {
    ...
    process.stdout.write('  ' + formatCurrency(s.period.close, s.currency, true, true, true) + ' ' + s.product_id.cyan)
    if (s.lookback[0]) {
    ...
    let orig_asset = s.orig_asset || s.start_asset
    if (orig_asset) {
...
*/
      let profit = (s.balance.asset - orig_asset) / orig_asset
      process.stdout.write(z(9, formatPercent(profit), ' ')[profit >= 0 ? 'green' : 'red'])
      /*
